...
def retrainCNN_470():
    ...
    """
    model = Sequential()
    model.add(Conv2D(32, kernel_size=3, activation='relu', input_shape=(32,32,3))) # create a 64 channel convoltional filter with size 3x3
    model.add(Conv2D(32, kernel_size=3, activation='relu')) # create a 64 channel convoltional filter with size 3x3
    model.add(Conv2D(64, kernel_size=3, activation='relu')) # create a 3x3 filter
    model.add(MaxPooling2D(pool_size=3)) # create a max pooling layer with a 3x3 receptive field
    ...
    model.fit(x_train, y_train, validation_data=(testSet, testLabels), epochs=5, verbose=1)
    ...
    def convert_to_subset(data, labels, class_labels):
        in_each_class = [(labels == new_class_label) for new_class_label in class_labels]
    ...
    tb_x_train, tb_y_train = convert_to_subset(x_train100, y_train100, [TIGER_ID, BUS_ID])
    tb_x_test, tb_y_test = convert_to_subset(testSet100, testLabels100, [TIGER_ID, BUS_ID])
    ...
    for i in range(len(trainable)):
        ...
        bus_tiger_model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
        ...
        print("The finetuned test accuracy on bus and tiger is {} with {} retrained layers".format(results[1], i))

    plt.plot(np.arange(len(results_for_all_runs)) + 1, results_for_all_runs)
    plt.title("Plot of accuracy versus number of trained layers, including the final one")
